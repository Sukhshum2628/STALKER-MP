...
| **008** | **Snapshot System (immutable snapshots)** | 🚧 **In progress** — Step 3 / 14 |
...
- **Sprint-008:** value types (Step 1), configuration (Step 2), and the immutable `SimulationSnapshot` + `ISnapshotView` (Step 3) implemented and verified; steps 4–14 (pool, capture seam, builder, queue, manager, engine adapter, wiring, diagnostics, hardening, integration, closure) remain.
...
- **330 / 330 tests passing** (Release x64) on **GCC** and **MSVC**.
